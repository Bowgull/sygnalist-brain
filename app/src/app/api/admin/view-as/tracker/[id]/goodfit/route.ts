...
/** POST /api/admin/view-as/tracker/:id/goodfit?client_id=xxx */
export async function POST(
  ...
  if (fetchErr || !entry) {
    logError(fetchErr?.message ?? `ViewAs GoodFit: tracker entry ${id} not found for client ${clientId}`, { severity: "warning", sourceSystem: "api.view-as.tracker.goodfit" });
  ...
  // Try to get the full job description from global_job_bank
  let jobDescription: string | null = null;
  if (entry.url) {
    const { data: bankJob } = await service
  ...
  if (!OPENAI_KEY) {
    logError("GoodFit: OPENAI_API_KEY not set", { severity: "error", sourceSystem: "api.view-as.tracker.goodfit" });
...
Output only the 3 paragraph blocks, each separated by a single blank line. Nothing else.`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!res.ok) {
        logError(`ViewAs GoodFit API returned ${res.status}`, { severity: "warning", sourceSystem: "api.view-as.tracker.goodfit" });
      ...
      if (!rawFit) {
        logError("ViewAs GoodFit response empty from AI", { severity: "warning", sourceSystem: "api.view-as.tracker.goodfit" });
        if (attempt < MAX_RETRIES) {
          await new Promise((r) => setTimeout(r, 1000 * Math.pow(2, attempt - 1)));
          continue;
        }
      ...
      try {
        ...
        logError(parseErr instanceof Error ? parseErr.message : "GoodFit parse failed", { severity: "warning", sourceSystem: "api.view-as.tracker.goodfit" });
        if (attempt < MAX_RETRIES) {
          await new Promise((r) => setTimeout(r, 1000 * Math.pow(2, attempt - 1)));
          continue;
        }
      ...
      // Save to tracker entry using service client
      await service
        .from("tracker_entries")
        .update({
          good_fit: goodFit,
      ...
      logError(err instanceof Error ? err.message : "AI request failed", {
        severity: "warning",
        sourceSystem: "api.view-as.tracker.goodfit",
