...
export async function POST(request: Request) {
  ...
  if (dbError) {
    logError(dbError.message, { sourceSystem: "api.admin.profile_create", userId: admin?.id });
    return error(dbError.message, 500);
  }

  logEvent("admin.profile_create", { userId: admin?.id, metadata: { target_profile_id: data.id, display_name: body.display_name } });
